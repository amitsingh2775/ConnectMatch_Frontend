...
const Preference = ({ setRoomID, preference, setPreference, onBack }) => {
  ...
  const handlePreferenceSubmit = (e) => {
    ...
    if (preference.preferences.length > 0) {
      setLoder(true);
      socket.emit("choose_preference", { preference: preference.preferences[0] }); // Send only preference
      socket.on("join_room", ({ roomID }) => {
        setLoder(false);
        setRoomID(roomID);
        sessionStorage.setItem("preference", preference.preferences[0]); // Store preference
      });
    } else {
  ...
  return (
    <div className="relative min-h-screen">
      <div
        className={`min-h-screen bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 text-white transition-all duration-700 ${loder ? "blur-md" : ""}`}
      >
        <div className="min-h-screen bg-[radial-gradient(circle_at_50%_120%,rgba(120,119,198,0.3),rgba(0,0,0,0))]">
          <div className="min-h-screen backdrop-blur-xl bg-black/20 flex items-center justify-center">
            <div className="w-[90%] sm:max-w-md mx-2 sm:mx-4">
              <button
                onClick={onBack}
                className="mb-4 text-white flex items-center gap-2 from-cyan-400 px-4 py-2 rounded-full hover:w-20 transition-colors"
              >
